{
  ...
  sops.secrets."yubikey/pin" = let
    step-ca = config.systemd.services.step-ca;
  in {
  ...
  systemd.services.step-ca.after = with config.systemd.services; [pcscd.name infnoise.name];
}
