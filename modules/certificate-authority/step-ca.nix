{ inputs, ... }:
{
  unify.nixos =
    ...
    in
    {
      ...
      # FIXME: re-enable upstream module when passwordFile is optional
      disabledModules = [
        "${inputs.nixpkgs}/nixos/modules/services/security/step-ca.nix"
      ];
      ...
      config = lib.mkIf config.services.step-ca.enable (
        let
          ...
          hasPasswordFile = cfg.passwordFile != null;
        in
        {
          ...
          systemd.services."step-ca" = {
            ...
            serviceConfig = {
              ...
              LoadCredential = lib.mkIf hasPasswordFile "password:${cfg.passwordFile}";

              ExecStart = [
                "" # override upstream
                (
                  "${lib.getExe cfg.package} /etc/smallstep/ca.json"
                  + lib.optionalString hasPasswordFile " --password-file \${CREDENTIALS_DIRECTORY}/password"
                )
              ];
