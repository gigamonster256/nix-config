...
let
  ...
  makeSystemConfig =
    ...
    withSystem hostConfig.system (
      ...
      in
      if hostPlatform.isLinux then
        { nixosConfigurations.${hostName} = cfg.flakes.nixpkgs.lib.nixosSystem builderArgs; }
      else if hostPlatform.isDarwin then
        { darwinConfigurations.${hostName} = cfg.flakes.nix-darwin.lib.darwinSystem builderArgs; }
      else
        throw "System type ${hostPlatform.system} not supported."
    );

  mkHomeConfiguration =
    pkgs: configName: homeConfig:
        ...
        ++ [
          (
            ...
            let
              ...
              username = if matches == null then configName else builtins.head matches;
            in
            {
              _file = ./.;
